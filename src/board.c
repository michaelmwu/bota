...
{ "General",  	"General discussion",		0,     2,     "all", DEF_REQUIRE,7, NULL, FALSE },
{ "Ideas",	"Suggestion for improvement",	0,     2,     "all", DEF_NORMAL, 18, NULL, FALSE }, 
{ "Announce", 	"Announcements from Immortals",	0,   102,     "all", DEF_NORMAL, 25, NULL, FALSE },
{ "Bugs",	"Typos, bugs, errors",		0,     1,     "imm", DEF_NORMAL, 15, NULL, FALSE },
{ "Personal", 	"Personal messages / Requests",	0,     1,     "all", DEF_EXCLUDE,20, NULL, FALSE },
{ "Clan", 	"Clan wars, requests, etc",     0,     0,     "all", DEF_NORMAL,12, NULL, FALSE },
{ "Help", 	"Have a question? Ask here.",   0,     0,     "all", DEF_NORMAL,6, NULL, FALSE },
{ "Heroes", 	"Special board for heroes",    101,  101,     "all", DEF_EXCLUDE,20, NULL, FALSE }

};

const char * szFinishPrompt = "(C)ontinue, (V)iew, (S)uspend, (P)ost it, (F)orget it?";
...
bool is_note_to (CHAR_DATA *ch, NOTE_DATA *note)
{
	...
	if(ch->clan && clan_table[ch->clan].name != '\0' &&
...
static void do_nwrite (CHAR_DATA *ch, char *argument)
{
    ...
    if(ch->in_arena)
    {
        send_to_char("Not while you're in the arena!\n\r",ch);
    }
	...
	sprintf (buf, "You are now %s a new note on the %s{x board.\n\r"
	...
	if (!ch->pcdata->in_progress->text) /* Are we continuing an old note or not? */
	{
		...
		case DEF_REQUIRE:
			sprintf (buf, "Hit return to continue, default recipient \"%s{x\".\n\r",
					  ch->pcdata->board->names);
			break;
		case DEF_INCLUDE:
...
static void do_nlist (CHAR_DATA *ch, char *argument)
{
	...
	send_to_char ("Notes on this board:{x\n\r"
	              "{BNum{W>       Author        Subject{x\n\r",ch);
	...
	for (p = ch->pcdata->board->note_first; p; p = p->next)
	{
		...
		if (is_note_to(ch,p))
		{
			...
			{
				sprintf (buf, "{B%3d{W>  {B%c   {Y%-13s {Y%s{x\n\r",
				               num,
...
void do_board (CHAR_DATA *ch, char *argument)
{
	...
	if (!argument[0]) /* show boards */
	{
		...
		for (i = 0; i < MAX_BOARD; i++)
		{
			...
			{ 
				sprintf (buf, "{B%2d{W>{x {G%12s{x [%s%4d{x] {Y%s{x\n\r", 
	...
	switch (ch->pcdata->board->force_type)
	{
		...
		case DEF_NORMAL: /* default field */
			if(!buf[0])
			{
				...
				sprintf (buf, "Assumed default recipient: %s{x\n\r", ch->pcdata->board->names);
				send_to_char(buf,d->character);
			}
			else ch->pcdata->in_progress->to_list = str_dup(buf);
			break;


		case DEF_REQUIRE: /* default field */
			ch->pcdata->in_progress->to_list = str_dup (ch->pcdata->board->names);
			sprintf (buf, "Default recipient: %s{x\n\r", ch->pcdata->board->names);
			send_to_char(buf,d->character);
...
void handle_con_note_subject (DESCRIPTOR_DATA *d, char * argument)
{
	...
	if (!buf[0])		
	{
		...
		send_to_char ("{BSubject{x: ", d->character);
	}
...
void handle_con_note_expire(DESCRIPTOR_DATA *d, char * argument)
{
	...
	if (!buf[0]) /* assume default expire */
		...
		if (!is_number(buf))
		{
			...
			send_to_char ("{BExpire{x: ",d->character);
			return;
		}
		else
		{
...
void handle_con_note_text (DESCRIPTOR_DATA *d, char * argument)
{
	...
	{
		write_to_buffer (d, "\n\r\n\r",0);
		send_to_char(szFinishPrompt, d->character);
		write_to_buffer (d, "\n\r", 0);
...
/*  Allan didn't like this...

	if (nc_strlen(buf) > MAX_LINE_LENGTH && !IS_IMMORTAL(d->character))
	{
...
void handle_con_note_finish (DESCRIPTOR_DATA *d, char * argument)
{
	...
	CHAR_DATA *ch = d->character;
		...
		switch (tolower(argument[0]))
		{
			...
			case 's':
				write_to_buffer (d, "Note suspended!\n\r",0);
				d->connected = CON_PLAYING;
				break;
...
int total_unread_notes(CHAR_DATA *ch)
{
    ...
    for (i = 0; i < MAX_BOARD; i++)
    {
        count = unread_notes(ch,&boards[i]);
        if(count > 0)
            total += count;
    }
