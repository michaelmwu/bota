...
CHAR_DATA *		char_list;
CHAR_DATA *		plr_list;
CHAR_DATA *		any_player_placeholder;
char *			help_greetinga;
char *			help_greetingb;
char *			help_greetingc;
char *			help_greetingd;
...
sh_int  		gsn_fishing;
sh_int  		gsn_vacillating_aura;
sh_int  		gsn_search;
sh_int  		gsn_accelerate_poison;
sh_int  		gsn_peace;
sh_int  		gsn_phase;
...
void boot_db( bool fCopyOver )
{
	...
	fBootDb		= TRUE;
        any_player_placeholder = new_char();

	for(count = 0; count < MAX_EVENT_HASH; count++)
	    event_hash[count] = NULL;
    }
    ...
    /* Init python stuff*/
    log_string(LOG_GAME, "Loading Python.");
    init_python();
	...
	{
	    log_string(LOG_ERR, "area.lst file not found!" );
	    exit( 1 );
	}
  ...
  /* OLC */     else if ( !str_cmp( word, "AREADATA" ) ) new_load_area(fpArea);
	...
	fix_exits( );
    log_string(LOG_GAME, "Fixing MobProgs." );
	fix_mobprogs( );
	fBootDb	= FALSE;
	strcat(boot_buf,"ng forge");
    log_string(LOG_GAME, "Converting Objects." );
	convert_objects( );				/* ROM OLC */
    log_string(LOG_GAME, "Setting up Python scripts." );
	setup_pyclass_scripts();
...
*/
		...
		pRoomIndex->owner = fread_string(fp);
                ...
                pRoomIndex->service_flags = fread_flag(fp);
...
void reset_room( ROOM_INDEX_DATA *pRoom, bool force )
{
    ...
    for ( pReset = pRoom->reset_first; pReset != NULL; pReset = pReset->next )
    {
        ...
        if(pReset->chance > 0 && pReset->chance < 1000 &&
           number_range(0,1000) > pReset->chance)
        {
            last = FALSE;
            continue;
        }
...
*/
             ...
             */
            {
                ...
                if ( pRoomIndexPrev
                    && IS_SET( pRoomIndexPrev->service_flags, SERVICE_PET_SHOP ) )
...
CHAR_DATA *create_mobile( MOB_INDEX_DATA *pMobIndex )
{
	...
	if (IS_SHIELDED(mob,SHD_PROTECT_EVIL))
	{
        ...
        if (IS_SHIELDED(mob,SHD_MANA))
        {
            af.where     = TO_SHIELDS;
            af.type      = skill_lookup("manashield");
            af.level     = mob->level;
            af.duration  = -1;
            af.location  = APPLY_NONE;
            af.modifier  = 0;
        ...
        if (IS_SHIELDED(mob,SHD_VACILLATING))
        {
            af.where     = TO_SHIELDS;
            af.type      = skill_lookup("vacillating aura");
            af.level     = mob->level;
            af.duration  = -1;
            af.location  = APPLY_NONE;
            af.modifier  = 0;
            af.bitvector = SHD_VACILLATING;
            affect_to_char( mob, &af );
        }
    }
	...
	for (i = 0; i < 3; i++)
	    mob->armor[i]	= interpolate32(mob->level,100,-100);
	mob->armor[3]		= interpolate32(mob->level,100,0);
	mob->race		= pMobIndex->race;
	mob->off_flags		= pMobIndex->off_flags;
	mob->imm_flags		= pMobIndex->imm_flags;
	mob->res_flags		= pMobIndex->res_flags;
    ...
    if(pMobIndex->pyclass != NULL)
        mob->pyobject = Character_FromChar(mob);
...
/*
    ...
    case ITEM_UNCOOKED_FOOD:
    case ITEM_MAGIC_CLOUD:
    case ITEM_BOAT:
    ...
    if(pObjIndex->pyclass != NULL)
        obj->pyobject = Object_FromObj(obj);
...
void *alloc_perm( int sMem )
{
    ...
    if(sMem > 35000) /* was: 10000 */
    {
      log_f("Alloc_perm: Assigning over 35000 bytes: %d at %p",
             sMem,pMem);
    }
...
void do_areas( CHAR_DATA *ch, char *argument )
{
    ...
    {
	sprintf( buf, "%-39s%-39s\n\r",
	    pArea1->credits, (pArea2 != NULL && pArea2->credits != NULL)
                               ? pArea2->credits : "" );
...
void do_memory( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf( buf, "Memory Per Area: %d.\n\r", sizeof(AREA_DATA) );
    send_to_char( buf, ch );

    sprintf( buf, "Memory Per MobProto: %d.\n\r", sizeof(MOB_INDEX_DATA) );
    send_to_char( buf, ch );

    sprintf( buf, "Memory Per Character: %d.\n\r", sizeof(CHAR_DATA) );
    send_to_char( buf, ch );
...
void do_dump( CHAR_DATA *ch, char *argument )
{
	...
	{
	    nMatch++;
	    fprintf(fp,"%p #%-4d %3d active %3d reset      %s\n",
		pObjIndex,
		...
		pObjIndex->reset_num,pObjIndex->short_descr);
            ...
            if(pObjIndex->item_type == ITEM_WEAPON)
            {
              fprintf(fp2,"-- %d %s (Level %d %s)\n"
                "Dice: %dd%d\n\n",
                vnum, pObjIndex->short_descr, pObjIndex->level,
    ...
    /* start printing out event data */
/*
    fp = fopen("event.dmp","w");

    fprintf(fp,"\nEvent Analysis\n");
    fprintf(fp,  "---------------\n");
    nMatch = 0;
    for (vnum = 0; nMatch < MAX_EVENT_HASH; vnum++)
	for (ev = event_hash[vnum]; ev != NULL; ev = ev->next_global )
	{
	    if(ev == NULL) continue;
	    nMatch++;
	    fprintf(fp,"%p %d\n", ev, ev->when); 
	}
    fprintf(fp, "Total: %3d   Memory: %d\n", 
	nMatch, nMatch * sizeof(EVENT));

    fclose(fp);

*/

    /* start printing out descriptor data */
    fp = fopen("sockets.dmp","w");

    fprintf(fp,"\nDescriptor Analysis");
    fprintf(fp,"\nNum State  Host      Player      Snooped");
    for (d = descriptor_list; d != NULL; d = d->next)
    {
        fprintf(fp,"%3d %2d    %-10s %-12s %3d\n",
		d->descriptor, d->connected, d->host,
		(d->character) ? d->character->name : "<none>",
		(d->snoop_by) ? d->snoop_by->descriptor : 0 );

        if ( d->character && d->character->pcdata
		 && d->character->pcdata->in_progress )
        {
	    fprintf(fp," -> Note:\nTo: %s\nSubject: %s\n",
		d->character->pcdata->in_progress->to_list,
		d->character->pcdata->in_progress->subject);
        }
    }
...
/*
 * Simple linear interpolation (0 to 32).
 */
int interpolate32( int level, int value_00, int value_32 )
{
...
{
    return v_min + (num - min) * (v_max - v_min) / (max - min);
}

int interpolate_r( int min, int max, int v_min, int v_max, int num )
{
    int val;
    val =  v_min + num * (v_max - v_min) / (max - min);
    return URANGE(v_min, val, v_max);
}
...
void doubleup_char( char ch, char *str, char *str2 )
{
    for ( ; *str != '\0'; str++ )
    {
        *str2 = *str;
...
/*  if (type & LOG_CRIT)
    {
	    ...
	    sprintf(bufee,"Critical: %s",bufew);
     }
*/
  if (type & LOG_ERR)
    {
	    sprintf( buf, "../log/%s.error", get_curdate() );
	    if( (log_file = fopen( buf, "a" )) != NULL )
	    {
	            strtime = ctime( &current_time );
		    strtime[strlen(strtime)-1] = '\0';
		    fprintf( log_file, "%s :: %s\n", strtime, bufew );
  ...
  if (type & LOG_SECURITY)
    {
	    sprintf( buf, "../log/%s.security", get_curdate() );
		    ...
		    fflush(log_file);
		    fclose( log_file );
		    sprintf(bufee,"Security: %s",bufew);
		}
    }
  if (type & LOG_CONNECT)
    {
...
void html_who()
{
    ...
    fprintf(fp,"<FONT COLOR=FFFFFF FACE='courier'>Lvl Name</FONT><BR>\n");
    ...
    for(d = descriptor_list; d != NULL; d = d->next)
    {
        ...
        if(ch->invis_level > 0) continue;
        if(ch->incog_level > 0) continue;

        fprintf(fp, "<FONT FACE='courier' COLOR=66FFFF>%3d</FONT> "
                    "<FONT FACE='courier' COLOR=FFFFFF>%s<br>\n",
            ch->level, ch->name);
    }
