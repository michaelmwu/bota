...
/*	pMobIndex->size			= size_lookup(fread_word(fp)); */
	...
	pMobIndex->pyclass_loc		= &str_empty[0];
	...
	for ( ; ; )
		...
		pMobIndex->say_descr[0]		= UPPER(pMobIndex->say_descr[0]);
             ...
             {
		pMobIndex->pyclass_loc		= fread_string( fp );
             }
	     else if ( letter == 'M' )
	     {
		MPROG_LIST *pMprog;
...
void load_objects( FILE *fp )
{
	...
	newobjs++;
        ...
        pObjIndex->pyclass_loc		= &str_empty[0];
	...
	switch (letter)
 	{
		...
		default:
            ...
            {
                pObjIndex->pyclass_loc = fread_string( fp );
            }
...
void convert_mobile( MOB_INDEX_DATA *pMobIndex )
{
    ...
    for (i = 0; i < 3; i++)
        pMobIndex->ac[i]         = interpolate32( level, 100, -100);
    pMobIndex->ac[3]             = interpolate32( level, 100, 0);  /* exotic */
