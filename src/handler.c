...
bool can_see_global( CHAR_DATA *ch, CHAR_DATA *victim );
...
bool is_friend(CHAR_DATA *ch,CHAR_DATA *victim)
{
    ...
    if (ch->in_arena && victim->in_arena)
        return (ch->arena_team == victim->arena_team);
...
{
	return IS_SET(clan_table[ch->clan].flags,GUILD_INDEPENDENT);
}

bool is_clan_independent(sh_int clan)
{
	return IS_SET(clan_table[clan].flags,GUILD_INDEPENDENT);
}

bool is_pkill(CHAR_DATA *ch)
{
	return IS_SET(clan_table[ch->clan].flags,GUILD_PKILL);
}

bool is_clan_pkill(sh_int clan)
{
	return IS_SET(clan_table[clan].flags,GUILD_INDEPENDENT);
}
...
void affect_join( CHAR_DATA *ch, AFFECT_DATA *paf )
{
	...
	if ( paf_old->type == paf->type )
	{
	    ...
	    paf->duration += UMIN(paf_old->duration,500);
...
void char_to_room( CHAR_DATA *ch, ROOM_INDEX_DATA *pRoomIndex )
{
    ...
    if ( ch->in_arena
        && (!pRoomIndex || !ROOM_IN_ARENA(pRoomIndex))
        && !IS_IMMORTAL(ch))
    {
        wiznet("$N unexpectedly left the arena.",
            ch,NULL,WIZ_BUGS,0,MAX_LEVEL);
        log_string(LOG_GAME,"%s left the arena!", ch->name);
        if(ch->was_in_room && ROOM_IN_ARENA(ch->was_in_room))
        {
	...
	bug_f( "Char_to_room: %s to room NULL.",
...
void equip_char( CHAR_DATA *ch, OBJ_DATA *obj, int iWear )
{
	...
	sprintf(buf, "Equip_char: %s already equipped (%d).",
               ch->name, iWear );
        bug(buf,0);
	return;
    }
...
void extract_char( CHAR_DATA *ch, bool fPull )
{
    ...
    if (!ch)
    {
        bug( "Extract_char: ch is NULL",0 );
        return;
    }

    if ( ch->in_room == NULL )
    {
	bug_f( "Extract_char: %s in NULL room. Attempting fix", ch->name );
...
/* send char to clan temple (room[2]) */
    ...
    if ( !IS_NPC(ch) )
    {
        ...
        if ( ch == plr_list )
        {
            plr_list = ch->plr_next;
        }
        else
        {
            CHAR_DATA *prev;

            for ( prev = plr_list; prev != NULL; prev = prev->plr_next )
            {
                if ( prev->plr_next == ch )
                {
                    prev->plr_next = ch->plr_next;
                    break;
                }
            }
            ...
            if ( prev == NULL )
            {
                bug( "Extract_char: player not found.", 0 );
                return;
            }
        }
    }
...
CHAR_DATA *get_char_world( CHAR_DATA *ch, char *argument )
{
    ...
    for ( wch = char_list; wch != NULL ; wch = wch->next )
    {
	if ( wch->in_room == NULL || !can_see_global( ch, wch ) 
...
CHAR_DATA *get_char_vnum( int vnum )
{
    ...
    for ( wch = char_list; wch != NULL ; wch = wch->next )
    {
        if ( !IS_NPC(wch) || wch->pIndexData->vnum != vnum || wch->in_room == NULL )
            continue;
	else
            return wch;
    }
...
/*
 * Find an obj in the world by vnum not on PCs.
 */
OBJ_DATA *get_obj_vnum_ground( int vnum )
{
    OBJ_DATA *obj;

    for ( obj = object_list; obj != NULL; obj = obj->next )
    {
        if ( obj->pIndexData->vnum == vnum && !obj->carried_by )
        {
	    return obj;
        }
    }
...
/*
 * Find an obj by vnum in the world
 */
...
/*
 * Count occurences of object not on PCs.
 */
int count_obj_vnum_ground( int vnum )
{
    OBJ_DATA *obj;
    int count;
...
bool room_is_dark( ROOM_INDEX_DATA *pRoomIndex )
{
    ...
    if ( ROOM_IN_ARENA(pRoomIndex) )
        return FALSE; /* ignore time of day in arena */
...
bool can_see_room( CHAR_DATA *ch, ROOM_INDEX_DATA *pRoomIndex )
{
    ...
    if (IS_SET(pRoomIndex->room_flags, ROOM_HEROES_ONLY)
    && IS_NPC(ch) && !(IS_SET(ch->act, ACT_PET) && ch->master
    && IS_HERO(ch->master)) )
	return FALSE;

    if (ROOM_IN_ARENA(pRoomIndex) && !ch->in_arena && !IS_IMMORTAL(ch))
        return FALSE;
    if (!ROOM_IN_ARENA(pRoomIndex) && ch->in_arena)
        return FALSE;
...
/* RT changed so that WIZ_INVIS has levels */
    ...
    {
	int chance;
	chance = get_skill(victim,gsn_sneak) - 15;
	chance += get_curr_stat(victim,STAT_DEX) * 3/2;
 	chance -= get_curr_stat(ch,STAT_INT) * 5/2;
...
{
    if(victim->ghost_level > get_trust(ch))
    {
        return FALSE;
    }
    ...
    return can_see_global(ch, victim);
}
...
char *item_type_name( OBJ_DATA *obj )
{
    ...
    case ITEM_IKEEP:		return "ikeeper";
    case ITEM_MAGIC_CLOUD:	return "magic cloud";
    case ITEM_FISHING_POLE:	return "fishing pole";
    case ITEM_UNCOOKED_FOOD:	return "uncooked food";
    }
...
char *shield_bit_name( int vector )
{
    ...
    if ( vector & SHD_VACILLATING	) strcat( buf, " vacillating"	);
    if ( vector & SHD_PEACE		) strcat( buf, " peace"	        );
    if ( vector & SHD_MANA              ) strcat( buf, " mana"         );
    return ( buf[0] != '\0' ) ? buf+1 : "none";
}
...
char *act_bit_name( int act_flags )
{
	...
	if (act_flags & ACT_IS_SPELLWIZ	) strcat(buf, " spellup_wiz");
	...
	if (act_flags & ACT_UPDATE_ALWAYS) strcat(buf," update_always");
        if (act_flags & ACT_NO_QUEST	) strcat(buf," noquest");
    }
    else
    {
	strcat(buf," player");
...
char *room_bit_name(int room_flags)
{
    ...
    if (room_flags & ROOM_ROAD		) strcat(buf, " road");
    ...
    if (room_flags & ROOM_LIMIT_PK	) strcat(buf, " limit_pk");
...
bool remove_voodoo(CHAR_DATA *ch)
{
	...
	if (obj->pIndexData->vnum != OBJ_VNUM_VOODOO) continue;
	if(obj->carried_by && IS_IMMORTAL(obj->carried_by)) continue;
...
int language_lookup( const char *name )
{
    ...
    for ( lnu = 0; lnu < MAX_LANGUAGE; lnu++ )
    {
        if ( language_table[lnu].name == NULL )
            break;
        if ( LOWER(name[0]) == LOWER(language_table[lnu].name[0])
        &&   !str_prefix( name, language_table[lnu].name ) )
            return lnu;
    }
...
{
    if(ch->in_arena)
        return arena.teams[ch->arena_team].hq;
    ...
    {
        if(ch->pcdata->recall)
            return get_room_index(ch->pcdata->recall);
    }

    return NULL;
}

void arena_pers(CHAR_DATA *ch)
{
    ;
}
