...
int find_spell( CHAR_DATA *ch, const char *name )
{
	...
	&&  skill_table[sn].spell_fun != spell_null)
	{
	    if ( found == -1)
		found = sn;
	    if (UMAX(get_trust(ch),ch->level) >= skill_table[sn].skill_level[ch->class]
...
int find_skill( CHAR_DATA *ch, const char *name )
{
    /* finds a skill the character can cast if possible */
    int sn, found = -1;
...
void say_spell( CHAR_DATA *ch, int sn )
{
	...
	if ( rch != ch )
      /*  act( ch->class==rch->class ? buf : buf2, ch, NULL, rch, TO_VICT ); */
	    act_new_2( ch->class==rch->class ? buf : buf2, ch, NULL, rch,
                TO_VICT, POS_RESTING, IS_IMMORTAL(ch) ? get_trust(ch): 0, 0 );
...
bool saves_spell( int level, CHAR_DATA *victim, int dam_type )
{
    ...
    save = 50 + ( victim->level - level) * 3 - victim->saving_throw * 2;
	...
	case IS_RESISTANT:	save += 10;	break;
	case IS_VULNERABLE:	save -= 10;	break;
    }
...
void do_cast( CHAR_DATA *ch, char *argument )
{
    ...
    || ( !IS_NPC(ch) && (UMAX(get_trust(ch),ch->level) < skill_table[sn].skill_level[ch->class]
    ||   		 ch->pcdata->learned[sn] == 0)))
    {
...
*/
   ...
   {
        send_to_char("It seems to have gotten stuck.\n\r",ch);
        return;
   }
	...
	else
	{
	    ...
	    act( "$p glows {Ggreen{x, then disappears from $n's inventory.", ch, obj, victim, TO_NOTVICT);
            log_string(LOG_GAME,"[iTrack] %s transferred %s to %s", ch->name, obj->short_descr, victim->name);
    ...
    if ( number_percent( ) >
         get_skill(ch,sn) + ((get_eq_char(ch,WEAR_WIELD) != NULL) ? 0 : 5)
         - (skill_table[sn].rating[ch->class] - 1)/3 )
    {
	send_to_char( "You lost your concentration.\n\r", ch );
	check_improve(ch,sn,FALSE,1);
	ch->mana -= mana / 2;
...
void spell_blindness( int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
    ...
    {
        send_to_char("That person is already blind!\n\r",ch);
	return;
    }
    ...
    af.location  = APPLY_HITROLL;
    af.modifier  = -4;
    af.duration  = number_range(level/40, level/15);
...
void spell_cancellation( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    ...
    if ((!IS_NPC(ch) && IS_NPC(victim) &&
	 !(IS_AFFECTED(ch, AFF_CHARM) && ch->master == victim) ) ||
        (IS_NPC(ch) && !IS_SET(ch->act,ACT_IS_HEALER) && !IS_NPC(victim)) )
    {
...
void spell_cause_light( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    ...
    damage_old( ch, victim, dice(1, 8) + level / 2, sn,DAM_HARM,TRUE);
    return;
}
...
void spell_cause_critical(int sn,int level,CHAR_DATA *ch,void *vo,int target)
{
    ...
    damage_old( ch, victim, dice(6, 10) + level - 6, sn,DAM_HARM,TRUE);
    return;
}
...
void spell_cause_serious(int sn,int level,CHAR_DATA *ch,void *vo,int target)
{
    ...
    damage_old( ch, victim, dice(8, 8) + level, sn,DAM_HARM,TRUE);
    return;
}
...
void spell_change_sex( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    ...
    af.type      = sn;
    af.level     = level;
    af.duration  = interpolate_r(5, 101, 1, 30, level);
...
void spell_charm_person( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    ...
    if( count > 10 )
    {
        send_to_char("You have enough followers already.\n\r",ch);
        return;
    }
    ...
    af.type      = sn;
    af.level	 = level;
    af.duration  = number_fuzzy( UMIN(level / 10, 8) );
    af.location  = 0;
    af.modifier  = 0;
...
void spell_cure_blindness(int sn,int level,CHAR_DATA *ch,void *vo,int target)
{
    ...
    if (check_dispel(level + 10,victim,gsn_blindness))
    {
...
void spell_cure_poison( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    ...
    if ( !is_affected( victim, gsn_poison ) && !is_affected(victim,gsn_accelerate_poison) )
    {
        if (victim == ch)
    ...
    if (check_dispel(level,victim,gsn_poison)
     || check_dispel(level,victim,gsn_accelerate_poison))
    {
...
void spell_curse( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    ...
    {
        send_to_char("That person is already cursed.\n\r",ch);
	return;
    }
    ...
    af.type      = sn;
    af.level     = level;
    af.duration  = IS_NPC(ch) ? 2*level : level/5;
    af.location  = APPLY_HITROLL;
...
void spell_dispel_magic( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    ...
    if (check_dispel(level,victim,skill_lookup("manashield")))
    {
	act("$n's manashield fades away.",
	    victim,NULL,NULL,TO_ROOM);
	found = TRUE;
    }
...
void spell_earthquake( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
	...
	if ( vch->in_room == ch->in_room )
	{
		...
		if (IS_AFFECTED(vch,AFF_FLYING))
		    ...
		    damage_old( ch,vch,level + dice(10, 12), sn, DAM_BASH,TRUE);
	    }
	    continue;
	}
...
void spell_faerie_fire( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    ...
    af.duration  = level;
    af.location  = APPLY_AC;
    af.modifier  = (level < 20) ? (5 * level/2) : (level * 2);
...
void spell_gate( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    ...
    ||   ch->in_arena
    ||   victim->in_arena
    ||   ROOM_IN_ARENA(victim->in_room)
    ||   IS_SET(victim->in_room->room_flags, ROOM_SAFE)
    ||   IS_SET(victim->in_room->room_flags, ROOM_LIMIT_PK)
    ||   IS_SET(victim->in_room->room_flags, ROOM_PRIVATE)
    ||   IS_SET(victim->in_room->room_flags, ROOM_SOLITARY)
    ||   IS_SET(victim->in_room->room_flags, ROOM_NO_RECALL)
    ||   IS_SET(ch->in_room->room_flags, ROOM_NO_RECALL)
    ||   IS_SET(victim->in_room->area->area_flags, AREA_NOPLAYERS)
    ||   victim->level > level + (IS_NPC(victim) ? 3 :  0 )
...
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    int gain = 80 + UMIN(victim->level * 3.2, victim->max_hit/8);

    if(ch && ch != victim && !IS_NPC(ch)) gain *= 0.65;

    if(ch && !strcmp(class_table[ch->class].name, "strider"))
    {
        gain *= 0.7;
/*        WAIT_STATE(ch,skill_table[sn].beats);*/
    }

    victim->hit = UMIN( gain + victim->hit, victim->max_hit );
    update_pos( victim );
...
void spell_heat_metal( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
   ...
   &&  !IS_SET(victim->imm_flags,IMM_FIRE))
   {
	    ...
	    obj_next = obj_lose->next_content;
            if ( number_range(1,1.5 * level) > obj_lose->level
	    &&   !saves_spell(level,victim,DAM_FIRE)
	    &&   !IS_OBJ_STAT(obj_lose,ITEM_NONMETAL)
	    &&   !IS_OBJ_STAT(obj_lose,ITEM_BOUND)
...
void spell_locate_object( int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
	...
	if ( in_obj->carried_by != NULL)
	{
            if (IS_IMMORTAL(in_obj->carried_by))
                if(get_trust(ch) < get_trust(in_obj->carried_by) )
                    continue;
...
void spell_magic_missile( int sn, int level, CHAR_DATA *ch,void *vo,int target)
{
    ...
    for(count = UMAX(1,level/15); count; count--)
    {
...
void spell_sleep( int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
    ...
    ||   saves_spell( level-14, victim,DAM_CHARM) )
	return;
...
void spell_weaken( int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
    ...
    af.where     = TO_AFFECTS;
    af.type      = sn;
    af.level     = UMIN(level, LEVEL_HERO + 30);
    af.duration  = level / 5;
    af.location  = APPLY_STR;
    af.modifier  = -1 * UMIN(level / 5, 10);
...
void spell_word_of_recall( int sn, int level, CHAR_DATA *ch,void *vo,int target)
{
   ...
   if(ch->in_arena)
    ...
    {
        log_string(LOG_GAME,"%s recalled to same room %d",ch->name, ch->in_room->vnum);
	return;
    }
...
void spell_accelerate_poison( int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    AFFECT_DATA *paf;

    if ( is_affected( victim, sn ) || saves_spell( level, victim,DAM_POISON) )
	return;
    ...
    if(paf->duration < 3)
    {
        send_to_char("The poison has nearly worn off already.\n\r",ch);
        return;
    }
    ...
    send_to_char( "The poison suddenly increases in potency!\n\r", victim);
    act("$n looks even worse than before.",victim,NULL,NULL,TO_ROOM);
    return;
}
...
void spell_shockwave( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
    ...
    send_to_char( "You send out a {Wblasting{x {Bshockwave{x!\n\r", ch);
    act("$n sends a horrific {Bsurge{x of power blazing throughout the room.",ch,NULL,NULL,TO_ROOM);
    ...
    if (number_percent() < 20 + (ch->level - victim->level) / 4 )
    {
        ...
        act("{iYou are stunned, and have trouble getting back up!{x",
            ch,NULL,victim,TO_VICT);
        act("{h$N is stunned by your shockwave!{x",ch,NULL,victim,TO_CHAR);
...
void spell_divine_wrath( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
    ...
    send_to_char( "You call down the gods to {Wsmite{x down your foes!\n\r", ch);
    act("$n calls down the gods!",ch,NULL,NULL,TO_ROOM);
    ...
    if (number_percent() < 15 + (ch->level - victim->level) / 4 )
    {
	    AFFECT_DATA af;
	    ...
	    if(saves_spell(level,victim,DAM_OTHER))
	    {
	        send_to_char("You fail to blind that person.\n\r",ch);
	        return;
	    }
...
void do_manashield ( CHAR_DATA *ch, char *argument )
{
        ...
        else if( !strcmp(argument,"off") )
        {
                ...
                {
                        send_to_char("Your manashield fades away.\n\r", ch);
                        act("$n's manashield fades away.",ch, NULL,NULL,TO_ROOM);
                        affect_strip(ch,gsn_manashield);
                }
                else
                        send_to_char("You don't have a manashield to turn off.\n\r", ch);
        }
        else
        {
                send_to_char("Syntax:  manashield <on|off>", ch);
        }
        return;
}
