...
typedef struct  bank_account_data	BANK_ACCOUNT_DATA;
typedef struct	grant_data		GRANT_DATA;
typedef struct	quest_data		QUEST_DATA;
typedef struct	pk_count_data		PK_COUNT_DATA;
...
typedef void SPELL_FUN	args( ( int sn, int level, CHAR_DATA *ch, void *vo,
				int target ) );
typedef	void EVENT_FUN	args( ( EVENT *e ) );
...
#define MAX_STAGE		   5
#define MAX_SECURITY		   15
#define MAX_LEVEL		  110
#define MAX_CLAN		17
#define MAX_RANK		6
#define LEVEL_HERO		   (MAX_LEVEL - 9)
#define LEVEL_IMMORTAL		   (MAX_LEVEL - 8)
#define MAX_RANDOM_ITEMS	2
#define MAX_RANDOM_MOBS		4

#define MAX_LANGUAGE		   4
#define MAX_ARENA_TEAMS		   2

#define ACTION_MUST_SEE		(A) /* Won't know unless you see at least one person */
#define ACTION_GLOBAL		(B)
#define ACTION_MUST_HEAR	(C)
#define ACTION_LOUD		(D)
#define ACTION_BATTLE_NODAM	(E)
...
#define PULSE_AREA		(PULSE_TICK * 2)
#define PULSE_ITEM		(PULSE_TICK * 3)
#define PULSE_MOB		(PULSE_TICK * 5)

#define GENERATE_LEVEL		50
...
#define GOLD_PER_SILVER		    100
#define PLATINUM_PER_GOLD	    100
#define PLATINUM_PER_SILVER	    (PLATINUM_PER_GOLD * GOLD_PER_SILVER)

#define TOTAL_MONEY_SILVER(ch) (ch->silver + (ch->gold*GOLD_PER_SILVER) \
				+ (ch->platinum * PLATINUM_PER_SILVER))

#define MONEY_VALUE_SILVER(x)			\
		((!x)?1:((x==1)?GOLD_PER_SILVER:PLATINUM_PER_SILVER))

#define MONEY_VALUE_NAME(x)			\
		((!x)?"silver":((x==1)?"gold":"platinum"))

#define BALANCE_MONEY(ch) 			\
        while (ch->silver >= GOLD_PER_SILVER)	\
        {					\
            ch->gold++;				\
            ch->silver -= GOLD_PER_SILVER;	\
        }					\
        while (ch->gold >= PLATINUM_PER_GOLD)	\
        {					\
            ch->platinum++;			\
            ch->gold -= PLATINUM_PER_GOLD;	\
        }					\
        if (ch->platinum > 32767)		\
            ch->platinum = 32767;
...
#define GUILD_PKILL			F /* immortal only clan */
...
struct	descriptor_data
{
    ...
    char **             pString;	/* OLC */
    int			editor;		/* OLC */
    bool		color;		/* Hackish temp variable */
    int			flags;
};
...
#define ff			2147483648	/* max value for signed long */
...
#define ACT_IS_SPELLWIZ		(X)
#define ACT_INDOORS		(Y)
#define ACT_IS_PRIEST		(Z)
#define ACT_IS_HEALER		(aa)
#define ACT_GAIN		(bb)
#define ACT_UPDATE_ALWAYS	(cc)
#define ACT_NO_QUEST		(dd)
...
#define OBJ_VNUM_MAGIC_CLOUD	   55
...
#define ITEM_EXIT		     36
#define ITEM_PIT		     37
#define ITEM_IKEEP                   38
#define ITEM_MAGIC_CLOUD             39
#define ITEM_FISHING_POLE            40
#define ITEM_UNCOOKED_FOOD           41
#define ITEM_TRAP                    42
#define ITEM_GENERIC                 43
...
#define ITEM_NOSAVE		(dd)
#define ITEM_BOUND		(ee)
...
#define ROOM_NO_CAMP		(Z)
#define ROOM_LIMIT_PK		(aa)

#define SERVICE_BANK		(A)
#define SERVICE_PET_SHOP	(B)
#define SERVICE_INN		(C)
#define SERVICE_VANTAGE_POINT	(D)
...
/* Secondary Temporary Acts */

#define PLT_QUESTOR             (A)
#define PLT_NO_PK               (B)
...
#define COMM_NOHERO          (dd)

/* comm2 */
#define COMM_TITLELOCK		(A)
#define COMM_NOOOC		(B)
#define COMM_NOIMP		(C)
#define COMM_HPBAR		(D)
#define COMM_NOBATTLESPAM	(E)
...
#define WIZ_SPAM		(T)
#define WIZ_BUGS		(U)
#define WIZ_NUMBERS		(V)
#define WIZ_TRANSFERS		(W)
#define WIZ_DESTRUCTION		(X)
#define WIZ_MEMCHECK		(Y)
#define WIZ_LOG			(Z)

#define MAX_EXITS 6 /* (or how many your mud has*/
...
struct	char_data
{
    ...
    sh_int		primary_lang;
    sh_int		language_known[MAX_LANGUAGE];
    bool		in_arena;
    sh_int		arena_team;
...
struct	pc_data
{
    ...
    MOB_INDEX_DATA *	questgiver;     /* who gave the quest */
    int			questpoints;
    int			nextquest;
    int			countdown;
    int			questobj;
...
#define QUEST_KILL_MOB		1
#define QUEST_GET_OBJ		2
#define QUEST_ESCORT		3

#define QUEST_PENDING		0
#define QUEST_IN_PROGRESS	1
#define QUEST_MUST_RETURN	4
#define QUEST_FINISHED		5
...
#define BANK_ACCOUNT_FROZEN	(A)
#define BANK_ACCOUNT_NOWITHDRAW	(B)
#define BANK_ACCOUNT_NODEPOSIT	(C)
#define BANK_ACCOUNT_NOTRANSFER	(D)
#define BANK_ACCOUNT_NOINTEREST	(E)
#define BANK_ACCOUNT_GROUP	(F)

#define MAX_BANK_RECORDS	10

#define BANK_TRANSACT_RECEIVE	1
#define BANK_TRANSACT_INTEREST	2
#define BANK_TRANSACT_CHARGE	3

struct bank_account_data
{
    BANK_ACCOUNT_DATA *next;
    bool valid;
    int bank_number;
    int interest;
    long balance;
...
struct	room_index_data
{
    ...
    sh_int		clan;
    int			items;
    sh_int		cover; /* not used */
    sh_int		elevation; /* not used */
    bool linked;
...
struct	skill_type
{
    ...
    {
        sh_int points_exp;
        sh_int points_practiced;
        sh_int points_taught;
        sh_int points_discovered;
    } stages[MAX_STAGE];
};
...
#define LANGUAGE_COMMON		 0
#define LANGUAGE_ELVISH		 1
#define LANGUAGE_DWARVISH	 2
#define LANGUAGE_ORCISH		 3


struct lang_type
{
    char *	name;
    char *	lname;	/* When listed */
};

#define KSKILL_INITIAL	(A)
#define KSKILL_CAP	(B)
...
extern sh_int  gsn_vacillating_aura;
extern sh_int  gsn_accelerate_poison;
extern sh_int  gsn_search;
extern sh_int  gsn_peace;
extern sh_int  gsn_phase;

extern sh_int  gsn_shockwave;
extern sh_int  gsn_divine_wrath;
...
#define BUF_OVERSIZED(abuf)	(abuf->size >= 15000)
#define ENTRE(min,num,max)	( ((min) < (num)) && ((num) < (max)) )
#define ENTRE_INC(min,num,max)	( ((min) <= (num)) && ((num) <= (max)) )
...
#define CLOSE_RESERVE()	fclose(fpReserve)

#define MANA_NEEDED(ach,msn)	(skill_table[msn].min_mana + \
(skill_table[msn].min_mana*((skill_table[msn].rating[ach->class]-1)*0.8)))
...
#define LEVEL_MIN_IMM(ch)	UMAX(get_trust((ch)),LEVEL_IMMORTAL)
...
#define CAN_SPEAK(ch,lang)	((ch)->language_known[lang] > 0 || IS_IMMORTAL(ch))
#define CH_ORIGINAL(ch)		((ch->desc && ch->desc->original)?ch->desc->original:ch )
...
#define IPADDR_TO_NUMBER(a,b,c,d) \
		(( a << 24 ) | ( b << 16  ) | ( c << 8 ) | ( d ))
...
#define GPERS(ch, looker)	( can_see_global( looker, (ch) ) ?	\
				( IS_NPC(ch) ? (ch)->short_descr	\
				: (ch)->name ) : "someone" )

#define SET_ALL_SIGNALS(sigx)		\
	{				\
	signal( SIGABRT, sigx );	\
	signal( SIGTERM, sigx );	\
	signal( SIGSEGV, sigx );	\
...
extern  char *	const			dir_name_rel	[];
extern	const	sh_int			rev_dir		[];
extern	const	sh_int			movement_loss	[SECT_MAX];
extern	const	struct	lang_type	language_table	[MAX_LANGUAGE];
...
extern		char			bug_buf		[];
extern		unsigned long		current_pulse;
...
#define HOUSE_DIR       "../houses/"  		/* saved houses */
#define TEMP_FILE	"../player/romtmp"
#define TEMP_FILE_HOUSE	"../houses/roomtmp"
...
int	nc_strlen	args( ( char *string ) );
void public_ch args( ( CHAR_DATA *ch, char *arg, char *ymsg, char *msg,
                       char *type, const int bitname) );
...
void	act2		args( ( const char *format, CHAR_DATA *ch, const void *arg1, const void *arg2, int type, long bits ) );
void	act_new		args( ( const char *format, CHAR_DATA *ch, const void *arg1, const void *arg2, int type, int min_pos) );
void	act_new_2	args( ( const char *format, CHAR_DATA *ch, const void *arg1, const void *arg2, int type, int min_pos, int min_level, long bits) );
...
void    log_f            args( ( char *, ... ) )
	__attribute__ ((format(printf,1,2)));
void    bug_f            args( ( char *, ... ) )
	__attribute__ ((format(printf,1,2)));

void	auto_shutdown	args( ( void ) );
void	foreign_speech	args( (CHAR_DATA *ch,const char *text,
			char *newstring ,int language) );
void	foreign_hearing args( (CHAR_DATA *ch,CHAR_DATA *sby,
			const char *text,char *transpose,int language) );
int	ofuscate_text	args( (const char *tochange, char *transpose,
			int percent) );
void	act_lang	args( ( const char *format, CHAR_DATA *ch, const void *arg1, const void *arg2, int type, int min_pos, int language) );
void	safe_coredump	args( ( void ) );
void	numberbar	args( ( char *buf, int width, int cur, int max ) );
void	print_last_command args ( ( void ) );
bool	file_exists	args( ( char *name ) );

/* csprintf.c */
int csprintf   ( char *buf, char *ptr, ... ) __attribute__ ((format(printf, 2, 3)));
int csnprintf  ( char *buf, int maxlen, char *ptr, ... ) __attribute__ ((format(printf, 3, 4)));

/* copyover.c */
void    copyover_recover args((void));

/* db.c */
void	reset_area      args( ( AREA_DATA * pArea, bool force ) );	/* OLC */
void	reset_room	args( ( ROOM_INDEX_DATA *pRoom, bool force ) );	/* OLC */
...
int	interpolate32	args( ( int level, int value_00, int value_32 ) );
int	interpolate	( int min, int max, int v_min, int v_max, int num );
int	interpolate_r	( int min, int max, int v_min, int v_max, int num );
...
void	doubleup_char	args( ( char ch, char *str, char *str2 ) );
void	quote_tildes( char *str, char *str2 );
...
bool    damage_old      args( ( CHAR_DATA *ch, CHAR_DATA *victim, int dam,
                                int dt, int class, bool show ) );
int     mana_damage      args( ( CHAR_DATA *ch, int mana ) );
...
bool    is_independent  args( (CHAR_DATA *ch) );
bool    is_clan_independent  args( (sh_int clan) );
...
int	count_obj_vnum_ground	args( ( int vnum ) );
OD *	get_obj_vnum		args( ( int vnum ) );
OD *	get_obj_vnum_ground	args( ( int vnum ) );
...
bool	is_clan_pkill	args( ( sh_int clan ) );
...
CD *	get_char_vnum	args( ( int vnum	) );
int	count_char_vnum	args( ( int vnum ) );
...
bool	can_see_global	args( ( CHAR_DATA *ch, CHAR_DATA *victim ) );
...
char *	one_case_argument	args( ( char *argument, char *arg_first ) );
void separate_char_end(char *argument, char ch, char *farg, char *endarg);

/* magic.c */
int	find_spell	args( ( CHAR_DATA *ch, const char *name) );
int	find_skill	args( ( CHAR_DATA *ch, const char *name) );
...
/* update.c */
void	advance_level	args( ( CHAR_DATA *ch, bool quiet ) );
...
int	quicklist_lookup args( ( const char *name ) );
...
#define         AREA_NOPLAYERS	(D)	/* IMMs only */
#define         AREA_NOSAVE	(E)	/* Don't save this area */
#define         AREA_NORECALL	(F)	/* No recall/teleport/gate */
#define         AREA_NOTELEPORT	(G)	/* No teleport/gate into  */
#define         AREA_NOWHERE	(H)	/* No teleport/gate into  */
...
extern		CHAR_DATA *		any_player_placeholder;

extern		MOB_INDEX_DATA *	mob_index_hash  [MAX_KEY_HASH];
extern		OBJ_INDEX_DATA *	obj_index_hash  [MAX_KEY_HASH];
extern		ROOM_INDEX_DATA *	room_index_hash [MAX_KEY_HASH];
extern		struct	arena_type	arena;

#define ARENA_STATUS_OPEN       0
#define ARENA_STATUS_WAITING    1
#define ARENA_STATUS_GAME       2
#define ARENA_STATUS_FINISHING  5

#define ARENA_DM                1
#define ARENA_CTF               2
#define ARENA_VT                3
#define ARENA_SIEGE		4
...
struct arena_type
{
    ...
    struct arena_team_type
    {
        ...
        int score;
        int players;
    } teams [MAX_ARENA_TEAMS];
} arena;

#define ROOM_IN_ARENA(_room)  (arena.status != ARENA_STATUS_OPEN && \
        ENTRE_INC(arena.vnum_start,  _room->vnum, arena.vnum_end))


#define CALL_SCRIPT(v,name)	script_call_noarg((v)->pyobject,name)

#define RANDOM_STRING(ar) ar[number_range(0,sizeof(ar)/sizeof(ar[0])-1)]
