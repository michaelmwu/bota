...
void song_update(void)
{
	...
	else
	{
	    ...
	    {
                if(song_table[channel_songs[1]].lyrics[channel_songs[0]] == '\0') 
                    strcpy(buf,"");
                else
	...
	if (obj->value[0] < 0)
	{
		...
		song_table[obj->value[1]].group,song_table[obj->value[1]].name);
            ...
            if (obj->carried_by != NULL)
                act(buf,obj->carried_by,obj,NULL,TO_ALL);
	    else if (obj->in_room && obj->in_room->people != NULL)
...
void do_play(CHAR_DATA *ch, char *argument)
{
	...
	/* scroll songs forward */
	juke->value[1] = juke->value[2];
	juke->value[2] = juke->value[3];
	juke->value[3] = juke->value[4];
	juke->value[4] = -1;


        act("$p stops playing music.",juke->carried_by,juke,NULL,TO_ALL);
	...
	sprintf(buf,"{B%s {Bhas the following songs available:{x\n\r",
	    juke->short_descr);
    ...
    if (!str_cmp(arg,"loud"))
    {
        ...
        {
            send_to_char("You're not allowed to use channels.\n\r",ch);
            return;
        }
