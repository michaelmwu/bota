...
void display_resets( CHAR_DATA *ch )
{
  ...
  "\n\r", ch );
	...
	ROOM_INDEX_DATA *pRoomIndex;
        char chances[MAX_STRING_LENGTH] = "";
	...
	sprintf( final, "[%2d] ", ++iReset );
        ...
        if(pReset->chance > 0)
        {
            sprintf(buf, "[%2d]  %.1f%%", iReset,
                (float)pReset->chance/10);
            strcat(chances, buf);
        }
	...
	case 'M':
	    ...
	    if ( !( pRoomIndex = get_room_index( pReset->arg3 ) ) )
	    {
            ...
            pMob = pMobIndex;
            csprintf( buf, "M[%5d] %-13.13s{x in room             R[%5d] %2d-%2d %-15.15s{x\n\r",
	...
	case 'O':
	    ...
	    if ( !( pRoomIndex = get_room_index( pReset->arg3 ) ) )
	    {
            ...
            csprintf( buf, "O[%5d] %-13.13s{x in room             "
                          "R[%5d]       %-15.15s{x\n\r",
	...
	case 'E':
	    ...
	    {
	    csprintf( buf,
		"O[%5d] %-13.13s{x in inv of S[%5d]       %-15.15s{x\n\r",
		pReset->arg1,
		pObj->short_descr,
		pMob->vnum,
		pMob->short_descr  );
	...
	case 'D':
	    ...
	    csprintf( buf, "R[%5d] %s door of %-19.19s{x reset to %s{x\n\r",
		pReset->arg1,
	...
	send_to_char( final, ch );
        ...
        {
            send_to_char("\n\r{BChances:{x\n\r",ch);
            send_to_char(" No.  Chance\n\r",ch);
            send_to_char("====  ======\n\r",ch);
            strcat(chances,"\n\r");
            send_to_char(chances,ch);
        }
    }
...
void do_resets( CHAR_DATA *ch, char *argument )
{
	...
	{
	    send_to_char(
		"{BResets:{x M = mobile, R = room, O = object, "
		"P = pet, S = shopkeeper\n\r", ch );
	    display_resets( ch );
	}
	else
	...
	/*
	 * Set chance (by 10ths) for a reset.
	 * ---------------
	 */
	else if ( !str_cmp( arg2, "chance" ) )
	{
	    int insert_loc = atoi( arg1 );
		...
		if ( !pReset )
		{
		    ...
		    return;
                ...
                if( !is_number(arg3) )
                {
		    send_to_char( "Invalid chance (must be a number 0 to 1000).\n\r", ch );
                    return;
                }
                ...
                if( !is_number(arg3) )
                {
		    send_to_char( "Must be between 0 and 1000 (100%).\n\r", ch );
                    return;
                }

                pReset->chance = atoi(arg3);
                sprintf(buf,"Reset load chance set to %.1f%%.\n\r",
                        (float)pReset->chance/10);
	    }
	  ...
	  || (!str_cmp( arg2, "obj" ) && is_number( arg3 )) )
	{
		...
		if ((pMob = get_mob_index( is_number(arg3)
                                          ? atoi( arg3 ) : 1 )) == NULL)
		{
		    send_to_char("Non-existant mob.\n\r",ch);
		    return;
		}

                if( !IS_BUILDER(ch, pMob->area) )
                {
                    send_to_char("You're not a builder for that area.\n\r",ch);
                    return;
                }
		...
		{
		    send_to_char( "Non-existant vnum.\n\r",ch);
		    return;
		}
                if(!IS_BUILDER(ch,pObj->area))
		{
...
void do_alist( CHAR_DATA *ch, char *argument )
{
    ...
    for ( pArea = area_first; pArea; pArea = pArea->next )
    {
	sprintf( buf, "[%3d] %-29.29s (%-5d-%5d) %-12.12s [%-2d][%-10.10s]\n\r",
	     pArea->vnum,
	     pArea->name,
	     pArea->min_vnum,
	     pArea->max_vnum,
...
void do_gedit( CHAR_DATA *ch, char *argument )
{
	...
	if ( ch->pcdata->security < 9 )
	{
	  send_to_char( "GEdit : Insufficient security to create new guilds.\n\r", ch );
  	  return;
	}

	gedit_create( ch, arg2 );
    ...
    else if ( arg[0] != '\0' )
    {
		...
		{
		        pClan = get_clan_data(clan_lookup( arg ));
		        if (pClan->name[0] == '\0')
			{
			    send_to_char( "That guild does not exist.\n\r", ch );
			    return;
    ...
    {
	send_to_char("Insufficient security to edit guilds.\n\r",ch);
	return;
    }
