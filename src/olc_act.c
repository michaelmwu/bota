...
int get_free_vnum(AREA_DATA *pArea);
...
const struct olc_help_type help_table[] =
{
    ...
    {	"shop",		shop_flags,	 "Shop attributes."		 },
    {	"service",	service_flags,	 "Service attributes."		 },
...
/* ROM specific bits: */
    ...
    {	"shields",	shield_flags,	 "Shield flags."		 },
...
bool check_range( int lower, int upper )
{
	...
	||   ( lower <= pArea->max_vnum && pArea->max_vnum <= upper ) )
        ...
        /* note: may be able to go up to ~2 billion; untested */
        if(lower < 0 || upper > 32760) return FALSE;
...
AEDIT( aedit_file )
{
    ...
    {
        send_to_char("Insufficient privileges\n\r",ch);
        return FALSE;
    }
...
REDIT( redit_show )
{
    ...
    sprintf( buf, "{rDescription{D:{x\n\r%s", pRoom->description );
    strcat( buf1, buf );

    sprintf( buf, "{rName{D:       {g[{x%s{g]{x\n\r{rArea{D:       {g[{x%5d{g]{x %s\n\r",
    ...
    sprintf( buf, "{rRoom flags{D: {g[{x%s{g]{x\n\r",
	    flag_string( room_flags, pRoom->room_flags ) );
    strcat( buf1, buf );

    sprintf( buf, "{rServices  {D: {g[{x%s{g]{x\n\r",
	    flag_string( service_flags, pRoom->service_flags ) );
    strcat( buf1, buf );
	...
	if ( ( pexit = pRoom->exit[door] ) )
	{
	    ...
	    sprintf( buf, "-{B%-5s{x to {g[{x%5d{g]{x {rKey{D: {g[{x%5d{g]{x ",
...
/*	pExit->vnum             = ch->in_room->vnum;    Can't set vnum in ROM */
	...
	char buf[MAX_STRING_LENGTH];
        ...
        {
            int vnum;
            if((vnum = get_free_vnum(pRoom->area)) < 1)
            {
                send_to_char("There are no available vnums in this area.\n\r",ch);
                return FALSE;
            }
            sprintf(arg, "%d", vnum);
        }
	...
	if ( arg[0] == '\0' || !is_number( arg ) )
	{
	    ...
	    send_to_char( "        [direction] dig auto\n\r", ch );
	    return FALSE;
	}
...
OEDIT( oedit_show )
{
    ...
    {
        sprintf(buf, "\n\rPython Script: %s     (Status: %s)\n\r",
                     pObj->pyclass_loc,
                     (pObj->pyclass == NULL) ? "not loaded" : "loaded");
        send_to_char(buf,ch);
    }
...
OEDIT( oedit_addaffect )
{
    ...
    {
        send_to_char( "Valid object affects are:\n\r", ch );
	show_help( ch, "apply" );
	return FALSE;
    }
...
OEDIT( oedit_addapply )
{
    ...
    {
        send_to_char( "Valid applys are (broken):\n\r", ch );
	show_help( ch, "apply" );
	return FALSE;
    }
...
MEDIT( medit_show )
{
    ...
    sprintf( buf, "{rName{D:        {g[{x%s{g]\n\r{rArea{D:        {g[{x%5d{g]{x %s\n\r",
	pMob->player_name,
	!pMob->area ? -1        : pMob->area->vnum,
	!pMob->area ? "No Area" : pMob->area->name );
    send_to_char( buf, ch );

    sprintf( buf, "{rAct{D:         {g[{x%s{g]{x\n\r",
	flag_string( act_flags, pMob->act ) );
    send_to_char( buf, ch );

    sprintf( buf,
    "{rVnum{D:        {g[{x%5d{g] {rSex{D:   {g[{x%s{g]   {rRace{D: {g[{x%s{g]{x\n\r",
	pMob->vnum,
    ...
    sprintf( buf,
    "{rLevel{D:       {g[{x%2d{g]    {rAlign{D: {g[{x%4d{g]  ",
        pMob->level,	pMob->alignment );
    send_to_char( buf, ch );

    sprintf( buf,
        "{rHitroll{D: {g[{x%2d{g] {rDam Type{D: {g[{x%s{g]{x\n\r",
    ...
    if ( pMob->group )
    {
	sprintf( buf, "{rGroup{D:       {g[{x%5d{g]{x\n\r", pMob->group );
	send_to_char( buf, ch );
    }

    sprintf( buf, "{rHit dice{D:    {g[{x%2dd%-3d{y+{x%4d{g]{x ",
	     pMob->hit[DICE_NUMBER],
	     pMob->hit[DICE_TYPE],
	     pMob->hit[DICE_BONUS] );
    send_to_char( buf, ch );

    sprintf( buf, "{rDam dice{D: {g[{x%2dd%-3d{y+{x%4d{g]{x   ",
	     pMob->damage[DICE_NUMBER],
	     pMob->damage[DICE_TYPE],
	     pMob->damage[DICE_BONUS] );
    send_to_char( buf, ch );

    sprintf( buf, "{rMana dice{D:   {g[{x%2dd%-3d{y+{x%4d{g]{x\n\r",
...
/* ROM values end */
    ...
    sprintf( buf, "{rAffected by{D: {g[{x%s{g]{x",
	flag_string( affect_flags, pMob->affected_by ) );
    send_to_char(buf,ch);
    col += nc_strlen(buf);
    if(col < 26)      for(;col < 27;col++) send_to_char(" ",ch);
    else if(col < 52) for(;col < 53;col++) send_to_char(" ",ch);
    else
    {
        send_to_char("\n\r",ch);
        col = 0;
    ...
    sprintf( buf, "{rShielded by{D: {g[{x%s{g]{x",
...
/* ROM values: */
    ...
    sprintf( buf,
    "{rArmor{D:       {g[{cpierce{D: {x%d  {cbash{D: {x%d  {cslash{D: {x%d  {cmagic{D: {x%d{g]{x\n\r",
	pMob->ac[AC_PIERCE], pMob->ac[AC_BASH],
	pMob->ac[AC_SLASH], pMob->ac[AC_EXOTIC] );
    send_to_char( buf, ch );

    sprintf( buf, "{rForm{D:        {g[{x%s{g]{x\n\r",
	flag_string( form_flags, pMob->form ) );
    send_to_char( buf, ch );

    sprintf( buf, "{rParts{D:       {g[{x%s{g]{x\n\r",
	flag_string( part_flags, pMob->parts ) );
    send_to_char( buf, ch );

    sprintf( buf, "{rImm{D:         {g[{x%s{g]{x",
    ...
    sprintf( buf,
    "{rSize{D:        {g[{x%-8s{g]    {rMaterial{D:    {g[{x%s{g]{x\n\r",
	flag_string( size_flags, pMob->size ),
        pMob->material );
    send_to_char( buf, ch );

    sprintf( buf,
    "{rStart Pos{D:   {g[{x%-8s{g]    {rDefault Pos{D: {g[{x%-8s{g]   {x",
	flag_string( position_flags, pMob->start_pos ),
	flag_string( position_flags, pMob->default_pos ) );
    send_to_char( buf, ch );

    sprintf( buf, "{rWealth{D: {g[{x%ld {ysilver{g]{x\n\r",
	pMob->wealth );
    send_to_char( buf, ch );
...
/* ROM values end */
    ...
    if ( pMob->spec_fun )
    {
	sprintf( buf, "{rSpec fun{D:    {g[{x%s{g]{x\n\r",
                  spec_name( pMob->spec_fun ) );
	send_to_char( buf, ch );
    }

    sprintf( buf, "{rShort descr{D: {x%s\n\r{rLong descr{D:{x\n\r%s",
	pMob->short_descr,
	pMob->long_descr );
    send_to_char( buf, ch );

    sprintf( buf, "{rDescription{D:\n\r{x%s", pMob->description );
    send_to_char( buf, ch );
    ...
    {
        sprintf(buf, "Python Script: %s     (Status: %s)\n\r",
                     pMob->pyclass_loc,
                     (pMob->pyclass == NULL) ? "not loaded" : "loaded");
        send_to_char(buf,ch);
    }
...
MEDIT( medit_damtype )
{
	...
	send_to_char( "To view a list of damage types, type '? weapon'.\n\r", ch );
	return FALSE;
    }
...
MEDIT( medit_level )
{
    ...
    {
        send_to_char("Level must be between 1 and 201.\n\r",ch);
        return FALSE;
    }
...
MEDIT( medit_shop )
{
	...
	if ( !pMob->pShop )
	{
        ...
        open_hour = atoi( arg1 );
        close_hour = atoi( argument );

        if(open_hour < 1 || open_hour > 23
        || close_hour < 1 || close_hour < 23)
        {
            send_to_char("Hours must be between 0 and 23.\n\r",ch);
            return FALSE;
        }

	pMob->pShop->open_hour = open_hour;
	pMob->pShop->close_hour = close_hour;
	...
	if ( !pMob->pShop )
	{
	    send_to_char( "MEdit:  Create a shop first. (shop assign).\n\r", ch );
	    return FALSE;
	}
	...
	pMob->pShop->profit_sell    = atoi( argument );
        ...
        if(pMob->pShop->profit_sell > 100)
        {
            send_to_char("Warning! Profit_sell should be under 100!\n\r",ch);
            send_to_char("Serious problems may arise - please reset it.\n\r",ch);
            send_to_char("{z (So reset it please){x\n\r",ch);
        }

        if(pMob->pShop->profit_buy < 85)
        {
            send_to_char("Warning! Profit_buy should be over 85!\n\r",ch);
            send_to_char("Serious problems may arise - please reset it.\n\r",ch);
        }
    ...
    if ( !str_cmp( command, "flags" ) )
    {
        ...
        if(arg1[0] == '\0')
        {
            send_to_char("Syntax: shop flags <flags>\n\r",ch);
            show_help( ch, "shop");
            send_to_char("\n\r",ch);
...
MEDIT( medit_script )
{
    ...
    if ( argument[0] == '\0' )
    {
	send_to_char( "Syntax:  script <module>:<class>\n\r", ch );
	return FALSE;
    }

    free_string( pMob->pyclass_loc );
    pMob->pyclass_loc = str_dup( argument );

    send_to_char( "Script class set.\n\r", ch);
    return TRUE;
}
...
MEDIT( medit_shields )
{
    ...
    send_to_char( "Syntax: shields [flag]\n\r"
		  "Type '? shields' for a list of flags.\n\r", ch );
    return FALSE;
}
...
REDIT( redit_service )
{
	...
	if ( (value = flag_value( service_flags, argument )) == NO_FLAG )
	{
		send_to_char( "Syntax: service [flags]\n\r", ch );
		return FALSE;
	}

        TOGGLE_BIT(room->service_flags, value);
	send_to_char( "Service flags toggled.\n\r", ch );
        return TRUE;
}
...
REDIT( redit_clan )
{
    ...
    {
        pRoom->clan = 0;
        send_to_char("Clan unassigned.\n\r",ch);
        return TRUE;
    }

    if( !(rclan = clan_lookup(argument)) )
    {
        send_to_char("Non-existant clan.\n\r",ch);
        return FALSE;
    }

    pRoom->clan = rclan;

    send_to_char ( "Clan assigned.\n\r", ch);
    return TRUE;
}
...
OEDIT( oedit_script )
{
    ...
    free_string( pObj->pyclass_loc );
    pObj->pyclass_loc = str_dup( argument );
...
AEDIT( aedit_shift )
{
    ...
    if ( !is_number( lower ) || lower[0] == '\0' )
    {
	send_to_char( "Syntax:  shift [#xlower]\n\r", ch );
	return FALSE;
    }
    ...
    ilower = atoi( lower );
    diff = ilower - pArea->min_vnum;
    iupper = pArea->max_vnum + diff;
...
GEDIT( gedit_show )
{
  ...
  sprintf(buf, "{rDescription{g:{x %s\n\r",
               pClan->description);
  add_buf(buffer, buf);

  sprintf(buf, "{rRecall Room{g:{x [%-6d]  {rMorgue Room{g:{x [%-6d]  {rClan Temple{g{x [%-6d]\n\r\n\r",
               pClan->room[0], pClan->room[1], pClan->room[2]);
  add_buf(buffer, buf);

  sprintf(buf, "{rPit{g:{x [%-6d]\n\r\n\r",
               pClan->pit);
  add_buf(buffer, buf);

  sprintf(buf, "{rFlags{g:{x      [%s]\n\r\n\r", flag_string( guild_flags, pClan->flags ) );
  add_buf(buffer, buf);

  sprintf(buf, "{r#    Rank                      Skill Associated with this Rank\n\r--------------------------------------------------------------{x\n\r");
  add_buf(buffer, buf);
  ...
  sprintf(buf,"{rCan Guild   {g:{x %s\n\r{rCan Deguild {g:{x %s\n\r{rCan Promote {g:{x %s\n\r{rCan Demote  {g:{x %s\n\r",
...
/** Function: gedit_pit
  * Descr   : Changes the vnum of the clan pit selected.
  * Returns : True/False if modified.
  * Syntax  : pit [#]
  * Written : v1.0 3/98
  * Author  : Gary McNickle <gary@dharvest.com>
