...
void save_mobile( FILE *fp, MOB_INDEX_DATA *pMobIndex )
{
    ...
    if (pMobIndex->pyclass_loc != '\0')
        fprintf( fp, "P %s~\n", pMobIndex->pyclass_loc);
 ...
 */
    ...
    if (pObjIndex->pyclass_loc != '\0')
        fprintf( fp, "P %s~\n", pObjIndex->pyclass_loc);
...
void save_rooms( FILE *fp, AREA_DATA *pArea )
{
    ...
    for( iHash = 0; iHash < MAX_KEY_HASH; iHash++ )
    {
		...
		if(pRoomIndex->service_flags)
                    fprintf( fp, "B %s\n",	fwrite_flag(pRoomIndex->service_flags, buf) );
    ...
    for( iHash = 0; iHash < MAX_KEY_HASH; iHash++ )
    {
        for( pRoomIndex = room_index_hash[iHash]; pRoomIndex; pRoomIndex = pRoomIndex->next )
        {
            if ( pRoomIndex->area == pArea )
            {
                ...
                {
                    if ( ( pExit = pRoomIndex->exit[door] )
                          && pExit->u1.to_room && pExit->rs_flags )
#if defined( VERBOSE )
			fprintf( fp,
                          "D 0 0 %d %d %ld *The %s door of %s is %s\n",
				pRoomIndex->vnum,
				pExit->orig_door,
				pExit->rs_flags,
...
void save_resets( FILE *fp, AREA_DATA *pArea )
{
	...
	case 'M':
	    ...
	    fprintf( fp, "M 0 %d %d %d %d %d *Load %s\n",
                pReset->chance,
	        pReset->arg1,
                pReset->arg2,
                pReset->arg3,
	...
	case 'O':
	    ...
	    fprintf( fp, "O 0 %d %d 0 %d *%s loaded to %s\n",
                pReset->chance,
	        pReset->arg1,
                pReset->arg3,
	...
	case 'P':
            pLastObj = get_obj_index( pReset->arg1 );
	    fprintf( fp, "P 0 %d %d %d %d %d *%s put inside %s\n",
                pReset->chance,
	        pReset->arg1,
	        pReset->arg2,
                pReset->arg3,
	...
	case 'G':
	    fprintf( fp, "G 0 %d %d 0 *%s is given to %s\n",
                pReset->chance,
	        pReset->arg1,
	        capitalize(get_obj_index( pReset->arg1 )->short_descr),
	...
	case 'E':
	    fprintf( fp, "E 0 %d %d 0 %d *%s is loaded %s of %s\n",
                pReset->chance,
	        pReset->arg1,
                pReset->arg3,
                capitalize(get_obj_index( pReset->arg1 )->short_descr),
	...
	case 'R':
	    ...
	    fprintf( fp, "R 0 0 %d %d *Randomize %s\n",
	        pReset->arg1,
                pReset->arg2,
                pRoom->name );
            break;
	...
	case 'G':
	    fprintf( fp, "G 0 %d %d 0\n", pReset->chance, pReset->arg1 );
            if ( !pLastMob )
            {
                sprintf( buf,
    ...
    for( iHash = 0; iHash < MAX_KEY_HASH; iHash++ )
    {
        for( pMobIndex = mob_index_hash[iHash]; pMobIndex; pMobIndex = pMobIndex->next )
        {
            if ( pMobIndex && pMobIndex->area == pArea && pMobIndex->pShop )
            {
                ...
                fprintf( fp, "%s\n", fwrite_flag(pShopIndex->flags, buf) );
            }
        }
    }
...
void save_area( AREA_DATA *pArea )
{
    ...
    aflags = pArea->area_flags & ~(AREA_LOADING|AREA_CHANGED|AREA_ADDED);

    if(aflags > 0)
        fprintf( fp, "Flags %s\n", fwrite_flag(aflags,buf));
...
void do_asave( CHAR_DATA *ch, char *argument )
{
	...
	for( pArea = area_first; pArea; pArea = pArea->next )
	{
		...
		{
		log_string(LOG_GAME,"asave-changed: %s '%24s'", pArea->file_name,pArea->name);
		}
		REMOVE_BIT( pArea->area_flags, AREA_CHANGED );
	    }
        }
