...
void do_gain(CHAR_DATA *ch, char *argument)
{
	...
	for (gn = 0; gn < MAX_GROUP; gn++)
	{
	    ...
	    &&  (group_table[gn].rating[ch->class] > 0 || IS_IMMORTAL(ch)))
	    {
		sprintf(buf,"%-18s %-5d ",
	...
	col = 0;
        ...
        for (sn = 0; sn < MAX_SKILL; sn++)
        {
            ...
            &&  (skill_table[sn].skill_level[ch->class] <= LEVEL_HERO || IS_IMMORTAL(ch))
	...
	if (group_table[gn].rating[ch->class] <= 0 && !IS_IMMORTAL(ch))
	{
	...
	if (skill_table[sn].skill_level[ch->class] > LEVEL_HERO && !IS_IMMORTAL(ch))
        {
    ...
    act("$N tells you '{aThere is no such skill or group...{x'",ch,NULL,trainer,TO_CHAR);
}
...
void do_spells(CHAR_DATA *ch, char *argument)
{
    ...
    for (sn = 0; sn < MAX_SKILL; sn++)
    {
      ...
      if (skill_table[sn].skill_level[ch->class] <= UMAX(LEVEL_HERO,get_trust(ch))  &&
	  skill_table[sn].spell_fun != spell_null &&
...
void check_improve( CHAR_DATA *ch, int sn, bool success, int multiplier )
{
	...
	if (number_percent() < chance)
	{
	    ...
	    send_to_char(buf,ch);
	    ch->pcdata->learned[sn]++;
	    gain_exp(ch,6 * skill_table[sn].rating[ch->class]);
	}
    }
	...
	if (number_percent() < chance)
	{
	    ...
	    gain_exp(ch,5 * skill_table[sn].rating[ch->class]);
	}
    }
}
